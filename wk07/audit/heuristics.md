...
**Evidence**:
- Plain language for example 'Add Task' and 'Delete' doesn't use technical jargon
- When adding a task it tells you simply that it was added successfuly rather than in technical status codes etc.
...
**Issue identified**: **Medium severity** — Add confirmation print statement or undo feature for delete.
...
**Rating**: 3/5 (Fair)
**Evidence**:
- Client-side `required` attribute means you can't submit nothing
- Server-side validation catches blank titles, but no way to stop accidentally deleting something

**Accessibility implication**: Benefits from preventing errors before they happen.

**Issue identified**: **Medium severity** — Delete button too easy to trigger accidentally.
...
**Accessibility implication**: Keyboard users could benefit from shortcuts.

**Issue identified**: **Low severity** — Add keyboard shortcuts (do Semester 2).
...
**Rating**: 5/5 (Excellent)
**Evidence**:
- No clutter - only shows what is necessary
- Edit form appears inline 
- Status messages dismiss automatically 
...
**Accessibility implication**: Cognitive users, first-time users may find it difficult to use without documentation.

**Issue identified**: **Low severity** — Add help tooltips or links to docs (do in Semester 2).
